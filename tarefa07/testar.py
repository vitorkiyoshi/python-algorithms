...
class Task(BaseTask):
    def teste_0_concatenacao(self):
        script = 'concatenacao.py'
        self.exists(script)
        self.test_case(script, join('testes', "concatenacao0.in"))
        self.test_case(script, join('testes', "concatenacao1.in"))

    def teste_1_concatenacao(self):
        script = 'concatenacao.py'
        self.exists(script)
        for i in range(2, 7):
            test_file = "concatenacao{}.in".format(i)
            self.test_case(script, join('testes', test_file))

    def teste_2_probabilidade(self):
        script = 'probabilidade.py'
        self.exists(script)
        self.test_case(script, join('testes', "probabilidade0.in"))
        self.test_case(script, join('testes', "probabilidade1.in"))

    def teste_3_probabilidade(self):
        script = 'probabilidade.py'
        self.exists(script)
        for i in range(2, 7):
            test_file = "probabilidade{}.in".format(i)
            self.test_case(script, join('testes', test_file))
